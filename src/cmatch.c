...
static int matchExpression(const char *file, const char *text, const char *selector) {
   CXIndex Idx;
   CXTranslationUnit TU;
   ...
   clang_matchAst(TU, (void*)0, text, &matchCallback, (void*)selector);
...
int main(int argc, char *const*argv) {
   ...
   while(1) {
      ...
      int c = getopt_long (argc, argv, "C:c:m:a:r:l:",
   ...
   if(matching && hasPattern && hasAstFile) {
      if(!hasSelector)
         strcpy(selector, "dump");
      return matchExpression(astFile, pattern, selector);
   }
   if(list && hasPattern) {
